...
class AppDelegate: NSObject, NSApplicationDelegate, NSOpenSavePanelDelegate {
    ...
    @IBAction func saveCLutAs(_ sender: NSMenuItem) {
        if savePanel.runModal() == NSApplication.ModalResponse.OK {
            if let path = savePanel.url?.path {
                let t:IMPCLut.LutType = type.hasPrefix("cube") ? .lut_3d : .lut_2d 
                (NSApplication.shared.keyWindow?.contentViewController as? ViewController)?.saveCLut(to: path, type: t)
            }
        }
    }
